...
%>		
	<div class="topnav">
	  ...
	  <form action="e_book.jsp" method="post">
	 ...
	 <% String search = request.getParameter("search"); 
	 	...
	 	%>
	 		...
	 		FROM User  
	 		INNER JOIN e_book    
	 		ON User.user_id = e_book.user_id ORDER BY Current_Time_Stamp DESC;
	 		</sql:query>  
	 	<% 
	 	}
	 	...
	 	%>
	 		...
	 		SELECT *  
	 		FROM User  
	 		INNER JOIN e_book using (user_id) where 
	 		 <%
	 		 try{
	 		 %>
	 		 	ebook_name="<%=search%>" or
		 		 user_name="<%=search %>" or
		 		 autor_name="<%=search %>"	
	 		<% 
	 		 }catch(Exception e){
